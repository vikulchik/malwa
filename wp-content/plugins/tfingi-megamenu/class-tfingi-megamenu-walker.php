...
if (!class_exists('Tfingi_Megamenu_Walker')){
    class Tfingi_Megamenu_Walker extends Walker_Nav_Menu_Edit {
...
*/
    ...
    function doDefaultTemplate(&$output, $item, $depth, $args, &$item_id, &$classes, &$title, &$megamenu_title, &$submenu_text, &$removed_args, &$original_title) {
        ...
        if ($depth == 0) {
            $item_id = $item->ID;
            $key = 'menu-item-tfingi-megamenu';
            $megamenuCheckboxValue = get_post_meta( $item_id, '_menu_item_tfingi_megamenu', true );
            $megamenuDescriptionValue = get_post_meta( $item_id, '_menu_item_tfingi_megamenu_description', true );
        }
        ?>
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); if($megamenuCheckboxValue != "") echo " megamenu-parent" ?>">
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ).' '.$megamenu_title; ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item' ); ?></span></span>
                        <span class="item-controls">
...
//                $megamenuCheckboxValue = get_post_meta( $item_id, '_menu_item_tfingi_megamenu', true );
//                $megamenuDescriptionValue = get_post_meta( $item_id, '_menu_item_tfingi_megamenu_description', true );
                if($megamenuCheckboxValue != "") $megamenuCheckboxValue = "checked='checked'";
