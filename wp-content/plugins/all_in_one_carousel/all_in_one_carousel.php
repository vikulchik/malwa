...
function all_in_one_carousel_activate() {
	...
	$sql0 = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "all_in_one_carousel_carousels` (
			`id` INT UNSIGNED NOT NULL AUTO_INCREMENT ,
			`name` VARCHAR( 255 ) NOT NULL ,
			PRIMARY KEY ( `id` )
			) ENGINE=MyISAM  DEFAULT CHARSET=utf8";
	
	$sql1 = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "all_in_one_carousel_settings` (
  ...
  `responsiveRelativeToBrowser` varchar(8) NOT NULL DEFAULT 'true',
	...
	$sql2 = "CREATE TABLE IF NOT EXISTS `". $wpdb->prefix . "all_in_one_carousel_playlist` (
	...
	$sql3 = "CREATE TABLE IF NOT EXISTS `". $wpdb->prefix . "all_in_one_carousel_texts` (
	  `id` int(10) unsigned NOT NULL auto_increment,
...
function all_in_one_carousel_plugin_menu() {
	...
	add_submenu_page( 'all_in_one_carousel', 'ALLINONE-CAROUSEL Carousel Settings', 'Carousel Settings', 'edit_posts', 'all_in_one_carousel_Settings', 'all_in_one_carousel_settings_page');
	add_submenu_page( 'all_in_one_carousel', 'ALLINONE-CAROUSEL Carousel Playlist', 'Playlist', 'edit_posts', 'all_in_one_carousel_Playlist', 'all_in_one_carousel_playlist_page');
...
jQuery(document).ready(function($) {
	...
	?>
		...
		jQuery('#upload_img_button_carousel_<?=$i?>').click(function() {
		 formfield = 'img';
		 the_i=<?=$i?>;
		...
		jQuery('#upload_thumbnail_button_carousel_<?=$i?>').click(function() {
		 formfield = 'thumbnail';
		 the_i=<?=$i?>;
		 tb_show('', 'media-upload.php?type=image&amp;TB_iframe=true');
		 return false;
	...
	jQuery("#form-playlist-all_in_one_carousel-<?=$i?>").submit(function(event) {

		/* stop form from submitting normally */
		event.preventDefault(); 
		
		//show loading image
		jQuery('#ajax-message-<?=$i?>').html('<img src="<?=plugins_url('all_in_one_carousel/images/ajax-loader.gif', dirname(__FILE__))?>" />');
		...
		//var data = {
			...
			//whatever: 1234
		//};
		var data ="action=all_in_one_carousel_update_playlist_record&security=<?php echo $all_in_one_carousel_update_playlist_record_ajax_nonce; ?>&"+jQuery("#form-playlist-all_in_one_carousel-<?=$i?>").serialize();

		// since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
		jQuery.post(ajaxurl, data, function(response) {
			//alert('Got this from the server: ' + response);
			//alert(jQuery("#form-playlist-all_in_one_carousel-<?=$i?>").serialize());
			var new_img = '';
			if (document.forms["form-playlist-all_in_one_carousel-<?=$i?>"].img.value!='')
				new_img=document.forms["form-playlist-all_in_one_carousel-<?=$i?>"].img.value;
			jQuery('#top_image_'+document.forms["form-playlist-all_in_one_carousel-<?=$i?>"].id.value).attr('src',new_img);
			jQuery('#ajax-message-<?=$i?>').html(response);
		});
	});
	<?php } ?>
